// Re-export shim — Orchestrator migrated to engine::actions.
pub use crate::engine::actions::{ActionKind, ActionRequest, RoutingDecision, RoutingError, Orchestrator};
...
mod tests {
    ...
    fn orchestrator_source_targets_configured_window_manager_directly() {
        let source = include_str!("actions/mod.rs");
        // The Orchestrator impl has no cfg(test) block; use the whole file.
