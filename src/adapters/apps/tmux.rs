...
// ---------------------------------------------------------------------------
// Shared utilities (pure formatting / CLI primitives)
// ---------------------------------------------------------------------------
...
/// Shared by `can_focus` and `move_decision` on both TopologyHandler impls.
...
/// (e.g. `["wezterm", "-e"]`).
pub(crate) fn tmux_from_client_pid(
    client_pid: u32,
    terminal_launch_prefix: Vec<String>,
) -> Option<Tmux> {
    let output = runtime::run_command_output(
...
pub(crate) fn tmux_nvim_in_current_pane(tmux: &Tmux) -> Option<u32> {
    let cmd = tmux_query(&tmux.session, "#{pane_current_command}").ok()?;
    if cmd != "nvim" && cmd != "vim" {
        return None;
    }
...
impl TerminalMuxProvider for TmuxMuxProvider {
    ...
    fn focused_pane_for_pid(&self, pid: u32) -> Result<u64> {
        ...
        let raw = tmux_query(&tmux.session, "#{pane_id}")?;
    ...
    fn pane_neighbor_for_pid(&self, pid: u32, pane_id: u64, dir: Direction) -> Result<u64> {
        ...
        let target = pane_target(pane_id);
        let output = runtime::run_command_output(
            "tmux",
    ...
    fn merge_source_pane_into_focused_target(
        ...
        let target_pane_raw = tmux_query(&tmux.session, "#{pane_id}")?;
        let target_pane_id = parse_pane_id(&target_pane_raw)
    ...
    fn active_foreground_process(&self, pid: u32) -> Option<String> {
        let tmux = resolve_tmux_for_terminal_pid(pid).ok()?;
        tmux_query(&tmux.session, "#{pane_current_command}").ok()
    }
}
...
impl TopologyHandler for TmuxMuxProvider {
    fn can_focus(&self, dir: Direction, pid: u32) -> Result<bool> {
        resolve_tmux_for_terminal_pid(pid)?.can_focus(dir, pid)
    }

    fn move_decision(&self, dir: Direction, pid: u32) -> Result<MoveDecision> {
        resolve_tmux_for_terminal_pid(pid)?.move_decision(dir, pid)
    }
    ...
    fn focus(&self, dir: Direction, pid: u32) -> Result<()> {
        resolve_tmux_for_terminal_pid(pid)?.focus(dir, pid)
    }

    fn move_internal(&self, dir: Direction, pid: u32) -> Result<()> {
        resolve_tmux_for_terminal_pid(pid)?.move_internal(dir, pid)
    }

    fn move_out(&self, dir: Direction, pid: u32) -> Result<TearResult> {
        resolve_tmux_for_terminal_pid(pid)?.move_out(dir, pid)
    }
...
impl TopologyHandler for Tmux {
    ...
    fn move_decision(&self, dir: Direction, _pid: u32) -> Result<MoveDecision> {
        let panes: u32 = tmux_query(&self.session, "#{window_panes}")?
            .parse()
            .unwrap_or(1);
        if panes <= 1 {
